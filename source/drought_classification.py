...
def main():
    ...
    if not os.path.exists('drought_classifier_plots'):
        os.mkdir('drought_classifier_plots')
    ############################################################
    ...
    for i, fname  in enumerate(fnames[:numFolds]):
        ...
        for j, sep in enumerate(sep_list):
            ...
            mse_baseline_test.append(clf_base.mse_score(y_pred_test, y_test))
            mse_baseline_train.append(clf_base.mse_score(y_pred_train, y_train))
            ...
            mse_knn_test.append(clf.mse_score(y_pred_test, y_test))
            mse_knn_train.append(clf.mse_score(y_pred_train, y_train))
    ...
    plt.savefig("drought_classifier_plots/CV_knn_mse_score.png")
    plt.close()
    print("Min MSE:", np.amin(mse_mean_score_knn_test))
    ...
    for test_set_size in test_set_sizes:
        ...
        mse_knn_test.append(best_clf.mse_score(y_pred_test, y_test))
        mse_knn_train.append(best_clf.mse_score(y_pred_train, y_train))
    ...
    plt.ylabel("Error Rate")
    plt.savefig("drought_classifier_plots/LC_knn_mse_score.png")
